        ...
        if (doc) {
            ...
            if (rnDoc.media.length > 0) {
                ...
                if (first_media.contentType.startsWith('image/')) {
                    og_tags.push({ tag_name: 'og:image', content: SERVER_URL + 'posts/media/' + first_media._id + '/1.jpg' });
...
exports.get_follower_posts = async (req, res, next) => {
    ...
    try {
        ...
        if (usr) {
            const followingLists = usr.followingLists;
            if (followingLists.length > 0) {
                const following_posts_of_usr = await Post.paginate({ user: { $in: followingLists }, isAvailable: true, postType: 'Follower' }, options);
                return res.status(200).send(following_posts_of_usr);
            }
...
exports.create_post = async (req, res, next) => {
    ...
    try {
        ...
        if (final_post) {
            switch (postType) {
                case 'Public': {
                    //emits post to all_posts subscriber
                    all_posts_socket.emit('post::created', final_post);
                    break;
                }
                case 'Only Me': {
                    console.log('In Only Me');
                    break;
                }
                case 'Follower': {
                    //for follower post
                    if (rnUser) {
                        ...
                        const friendLists = rnUser.followerLists;
                    ...
                    const followerLists = rnUser.followerLists;
                    ...
                    for (let j = 0, len = followerLists.length; j < len; j++) {
                        let follower = await User.findById(followerLists[j]);
                    ...
                    if (background_playerIds.length >= 1) {
                        ...
                        if (rnNoti) {
                            const description = rnNoti.createdBy.name + ' created a new post.';
                            let pic_path = '';
                            if (rnNoti.media) {
                                if (rnNoti.media.contentType.startsWith('video/')) {
                                    pic_path = SERVER_URL + 'posts/media/' + rnNoti.media._id + '/thumbnail';
...
exports.search_posts = async (req, res, next) => {
    ...
    try {
        const searched_posts = await Post.esSearch({ query: match_query });
        if (searched_posts) {
            ...
            //paginate posts
            const posts = await Post.paginate({ _id: { $in: all_posts_uniq }, postType: 'Public' }, options);
